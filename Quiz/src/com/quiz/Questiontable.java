...
public class Questiontable {
	...
	public static void main(String[] args) throws SQLException { // exception for Sql error
		...
		// inserting question into table with all option and their answers

		PreparedStatement pst = connection
				.prepareStatement("insert into que_ans(question,op1,op2,op3,op4,answer)values(?,?,?,?,?,?)");
		// que1
		pst.setString(1, "In which language MYSQL is written?");
		...
		pst.addBatch();
		// que2
		pst.setString(1, ":To see the list of options provided by MYSQL which of the following command is used?");
		...
		pst.setString(1, "In which form MYSQL query results are displayed?");
		...
		pst.setString(1, "To see all the databases which command is used?");
		...
		pst.addBatch();
		// que6
		pst.setString(1, "In the following statement 'USE student;' , what do you mean by the 'student'?");
		...
		pst.addBatch();
		// que7
		pst.setString(1, "ALTER command is a type of which SQL command?");
		...
		pst.addBatch();
		// que8
		pst.setString(1, " Is a semicolon necessary after every query?");
		...
		pst.addBatch();
		// que9
		pst.setString(1, "From all the clauses in MYSQL what task is performed by the DISTINT clause?");
		...
		pst.addBatch();
		// que10
		pst.setString(1, " Which key is commonly known as a subset of a super key?");
